...
const LoginAndAttendancePage = () => {
  ...
  const [filteredEntries, setFilteredEntries] = useState<
    Array<{ name: string; contactNumbers: string[] }>
  >([]);
    ...
    <div className="bg-gray-100 min-h-screen flex flex-col">
      <main className="flex-grow container mx-auto p-8 flex items-center justify-center">
        <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
          {notification && (
            <div
              className={`mb-4 p-3 rounded ${notification.type === 'success' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}
            >
          ...
          {!isLoggedIn ? (
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <button
                ...
                className={`w-full bg-blue-600 text-white px-6 py-2 rounded hover:bg-blue-700 transition duration-300 ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
      ...
      {showRegistrationForm && (
        <RegistrationForm onClose={() => setShowRegistrationForm(false)} isAttendancePage={true} />
      )}
    </div>
  );
};
